...
Railway is the accepted target for the NestJS API, PostgreSQL, and future backend services, with isolated production and test environments. Render is historical and the old Render database does not need recovery for the current restoration work. Production restoration, backup/restore verification, observability, and the operational runbook remain active delivery work under [[ADR 0026 - Use Railway For Backend Hosting]] and [[Deployment Guide]].
